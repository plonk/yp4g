...
protected:
  //バインド
  bool pbind(int port, ip_address bind_addr){
	...
	{
		int temp = 1;
		::setsockopt(sock_, SOL_SOCKET, SO_REUSEADDR, (char *)&temp, sizeof(temp));
	}

    sockaddr_in addr;
    addr.sin_family= AF_INET;
    addr.sin_port= htons(port);
	addr.sin_addr= bind_addr.addr();
...
{
public:
  gimite::socket_address last_addr;
  //すぐにはバインドしないコンストラクタ
  server_stream_socket()
    : bound_socket(SOCK_STREAM){}
  //すぐにバインドするコンストラクタ
  explicit server_stream_socket(int port, ip_address bind_addr, int backlog= 5)
      : bound_socket(SOCK_STREAM){
    bind(port, bind_addr, backlog);  
  }
  //既存のソケットハンドルを使うコンストラクタ
  server_stream_socket(socket_t sock, bool is_bound)
    : bound_socket(SOCK_STREAM, sock, is_bound){}
  //バインド
  bool bind(int port, ip_address bind_addr, int backlog= 5){
    if (!pbind(port, bind_addr)) return false;
...
#else
	last_addr.ip = ip_address(sa.sin_addr.s_addr);
#endif
    last_addr.port = sa.sin_port;
